from utils import currEpoch, plotLoss, plotPointCloud, saveModel

def trainingLoopAE(model, num_epoch, criterion, optimizer, loss_dict, path):
  """
  A universal training loop for Autoencoder to optimize any loss function
  ...
  if curr_epoch is not -1:
    model = torch.load(path + '/' + str(curr_epoch))
    model.eval()

  for epoch in range(epochs_left):  # loop over the dataset multiple times
    running_loss = 0.0

    for data in training_generator_ae:
    ...
    if epoch % 10 == 0:    # print every 10 mini-batches
...
def noiseFunc(mu, sigma, batch_size):
    """
    ...
    """
    noise = np.random.normal(mu, sigma, (batch_size, 128))
...
def trainingLoopGAN(training_generator, generator, discriminator,
                    model_name, num_epoch, optimizer_g, optimizer_d, 
                    loss_dict, path, ae=None, mu=0, sigma=0.2, 
  ...
  for epoch in range(num_epoch):  # loop over the dataset multiple times
    ...
    for data in training_generator:
      ...
      for _ in range(discriminator_boost):
        ...
        # forward + backward + optimize
        noise = noiseFunc(mu, sigma, data.shape[0])
        ...
        if 'Latent' in generator.name:
          grad_penal = compute_gradient_penalty(discriminator, data, outputs)
          loss_d = loss_d + lambda_gp * grad_penal
        optimizer_d.step()

      loss_dict[discriminator.name].append(loss_d)
      ...
      loss_dict[generator.name].append(loss_g)  
